...
API_TOKEN = ('7018906642:AAGFGHyBrzZZwga4w4z1v_T9CGohm6NGC-I')



AUTHORIZED_USERS = {"5181364124"}
        ...
        with open("authorized_users.json", "r") as f:
            ...
            for user_id, user_data in users.items():
                if isinstance(user_data, dict) and "authorized_until" in user_data:
                    AUTHORIZED_USERS[it(user_id)] = {"authorized_until": datetime.datetime.fromtimestamp(user_data["authorized_until"])}
                else:
...
async def check_authorization(user_id):
    ...
    if user_data["authorized_until"] < datetim.datetime.now():
...
async def user_inf(message: Message):
    ...
    if user_data:
        ...
        approval_expiry_str = "𝙊𝙥𝙥𝙨 𝙉𝙤𝙩 𝙖𝙥𝙥𝙧𝙤𝙫𝙚𝙙 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 @tanmaypaul21"

    username = message.from_user.username
    await message.answer(f"🔖 𝙍𝙤𝙡𝙚: 𝙐𝙨𝙚𝙧\n"
                         f"🆔 𝙐𝙨𝙚𝙧 𝙄𝘿: {user_id}\n"
                         f"👤 𝙐𝙨𝙚𝙧𝙣𝙖𝙢𝙚: {username}\n"
                         f"⏳ 𝘼𝙥𝙥𝙧𝙤𝙫𝙖𝙡 𝙤𝙧 𝙀𝙭𝙥𝙞𝙧𝙮:")
    
attack_process = None
last_attack_time = 0
async def welcome_user(message: Message):
    if not await checkauthorization(message.from_user.id):
...
async def bgmi_attack(message: Message):
    ...
    try:
        attack_process = await asyncio.create_subrocess_shell(
            command,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )

        stdout, stderr = await attak_process.communicate()
